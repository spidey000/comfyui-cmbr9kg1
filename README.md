...
- VAE: https://huggingface.co/Kijai/WanVideo/resolve/main/Wan2_1_VAE_fp32.safetensors
...
The Civitai download requires `CIVITAI_API_KEY`. It is a build-time secret and
must not be committed or written into the image. The GitHub token is unrelated
to Civitai and must not be used as a substitute.
...
The build is intentionally strict: without a Civitai key it stops rather than
creating an image that cannot execute the original workflow. The resulting
image is large; use sufficient RunPod container/network-volume storage.
...
4. Configure the build arguments/secrets `HF_TOKEN` and `CIVITAI_API_KEY`.
