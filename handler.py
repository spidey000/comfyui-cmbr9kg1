...
def handler(job):
    ...
    try:
        if validated_data.get("lora_downloads") is not None:
            ephemeral_loras = list(download_loras(validated_data["lora_downloads"], validated_data.get("civitai_token")))
            inject_loras(workflow, ephemeral_loras)
        ...
        for name, _ in ephemeral_loras:
            try:
                os.remove(os.path.join("/runpod-volume/models/loras", name))
            except OSError:
                pass
